...
<body>
    ...
    if(isset($_POST['login'])){
        $msg = '';
        $post_data = $_POST;
        $check_user = "SELECT id, first_name, last_name, email FROM `users` WHERE email='" . $post_data["email"] . "' AND password='" . md5($post_data["password"]) . "'";
	...
	<div class="container">
		...
		<div class="l-inner">
			...
			<form method="POST">
				...
				<div class="form-group ac-bx d-flex align-items-sm-center justify-content-between">
					<div class="check-bx">
					   <div class="custom-control custom-checkbox">
						 <input type="checkbox" class="custom-control-input" id="customControlAutosizing">
						 <label class="custom-control-label" for="customControlAutosizing">Save Password</label>
					   </div>
					</div>
					<a href="#" class="lnkForgot">Forgot Password?</a>
				 </div>
                <div class="submit-btn">
                    <button type="submit" name="login" value="Login">Login</button>
				</div>
				<p class="p-reg">Don't have an account? <a href="register.php">Register here</a>, it takes less than a minute.</p>
			</form>
		</div>
	</div>
	</div>
